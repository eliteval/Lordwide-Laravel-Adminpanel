    ...
    <div class="row">
        <div class="col-9 mx-auto">
            <div class="card card-body mt-4">
                <h6 class="mb-0">{{ __('Edit Item') }}</h6>
                <p class="text-sm mb-0">{{ __('Edit item') }}</p>
                <hr class="horizontal dark my-3">
                <form wire:submit.prevent="editItem" action="#" method="POST">
                    <div class="avatar avatar-xxl position-relative">
                        <div class="@error('photo') has-danger @enderror">
                            ...
                            <span
                                ...
                                @else
                                    <img src="{{ $item->pictureUrl() }}" alt="Profile Photo">
                                @endif
                            </span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="@error('item.name') has-danger @enderror">
                            <input wire:model.lazy="item.name" type="text"
                                class="form-control @error('item.name') is-invalid @enderror" id="itemName">
                        </div>
                        @error('item.name') <div class="text-danger text-xs">{{ $message }}</div> @enderror
                    </div>
                    <div class="mt-3">
                        <label for="itemExcerpt" class="form-label">{{ __('Item Excerpt') }}</label>
                        <div>
                            <input wire:model.lazy="item.excerpt" type="text" class="form-control" id="itemtExcerpt">
                        </div>
                    </div>
                    <div>
                        ...
                        <div wire:ignore>
                            ...
                            " x-on:quill-text-change.debounce.1000ms="@this.set('item.description', $event.detail)">
                                {!! $item->description !!}
                            </div>
                        </div>
                    </div>
                    <div>
                        <div wire:ignore>
                            ...
                            <div class="@error('item.category_id') has-danger @enderror">
                                <select wire:model="item.category_id"
                                    class="form-control @error('item.category_id') is-invalid @enderror"
                        ...
                        @error('item.category_id') <div class="text-danger text-xs">{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        ...
                        <div wire:ignore>
                            @php
                                $selected = json_encode(App\Http\Livewire\LaravelExamples\EditItem::getExistingTags());
                            @endphp
                            @php
                                $array = [];
                            @endphp
                    ...
                    <div>
                        ...
                        <div class="@error('status') has-danger @enderror d-flex flex-column">
                            <div class="form-check">
                                <input wire:model="item.status" type="radio"  class="form-check-input" name="status" id="published"
                                    value="Published">
                                <label class="custom-control-label" for="published">{{ __('Published') }}</label>
                            </div>
                            <div class="form-check">
                                <input wire:model="item.status" type="radio"  class="form-check-input" name="status" id="draft" value="Draft">
                                <label class="custom-control-label" for="draft">{{ __('Draft') }}</label>
                            </div>
                            <div class="form-check">
                                <input wire:model="item.status" type="radio"  class="form-check-input" name="status" id="Active" value="Active">
                    ...
                    <div>
                        <label class="mt-4 pl-0 form-label">{{ __('Item Options') }}</label>
                        <div class="form-check">
                            <div>
                                <div class="custom-control custom-checkbox">
                                    <input wire:model="options.{{ 0 }}" class="form-check-input" name="options[]"
                                        id="optionFirst" type="checkbox" value="0"
                                        {{ ($item->options !== null && in_array('0', $item->options)) ? 'checked' : ''}}>
                                    <label class="custom-control-label" for="optionFirst">{{ __('First') }}</label>
                                </div>
                                <div class="custom-control custom-checkbox">
                                    <input wire:model="options.{{ 1 }}" class="form-check-input" name="options[]"
                                        id="optionSecond" type="checkbox" value="1"
                                        {{ ($item->options !== null && in_array('1', $item->options)) ? 'checked' : ''}}>
                                    <label class="custom-control-label" for="optionSecond">{{ __('Second') }}</label>
                                </div>
                                <div class="custom-control custom-checkbox">
                                    <input wire:model="options.{{ 2 }}" class="form-check-input" name="options[]"
                                        id="optionThird" type="checkbox" value="2"
                                        {{ ($item->options !== null && in_array('2', $item->options)) ? 'checked' : ''}}>
                    ...
                    <div>
                        ...
                        <div>
                            <input wire:model="item.date" class="form-control datetimepicker" type="text"
