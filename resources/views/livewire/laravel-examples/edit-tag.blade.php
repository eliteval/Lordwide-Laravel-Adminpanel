    ...
    <div class="row">
        <div class="col-9 mx-auto">
            <div class="card card-body mt-4">
                ...
                <form wire:submit.prevent="editTag" action="#" method="POST">
                    <div>
                        <label for="tagName" class="form-label">{{ __('Tag Name') }}</label>
                        <div class="@error('tag.name')has-danger @enderror">
                            <input wire:model="tag.name" type="text"
                                class="form-control @error('tag.name')is-invalid @enderror" id="tagName">
                        </div>
                        @error('tag.name') <div class="text-danger text-xs">{{ $message }}</div>@enderror
                    </div>
                    <div>
